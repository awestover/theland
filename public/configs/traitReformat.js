...
let stats = [];
let names = [];
for(let i in allStats["personals"])
{
	stats.push(allStats["personals"][i].speed)
	names.push(i)
}
...
console.log(names);
for(let i in stats){
	revisedStats.push(1+Math.round(10*(stats[i]-min)*9/(max-min))/10)
}

for(let i in stats){
	console.log(names[i]+" "+ revisedStats[i]);
}
